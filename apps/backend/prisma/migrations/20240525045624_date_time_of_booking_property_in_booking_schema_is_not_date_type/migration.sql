/*
  ...
  - Changed the type of `dateTimeOfBooking` on the `Booking` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
...
DROP COLUMN "dateTimeOfBooking",
ADD COLUMN     "dateTimeOfBooking" TIMESTAMP(3) NOT NULL;
